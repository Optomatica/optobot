...
module MessengerHelper
  ...
  def self.receipt_template(page_access_token, response, user_project)
     ...
     elements = Array(JSON.parse(response[:element]))
     prices = Array(JSON.parse(response[:price]))
     quantitites = Array(JSON.parse(response[:quantity]))
     image_urls = Array(JSON.parse(response[:image_url]))
     @body = { "attachment" =>{
             "type" => "template",
             "payload" => {
